...
@section('title', 'Keranjang Belanja - ' . \App\Models\Setting::get('site_name', 'Batik Jambi Berkah'))
    ...
    <div class="max-w-4xl mx-auto">
        ...
        @if(count($cart) > 0)
            <div class="grid grid-cols-1 gap-6">
                @foreach($cart as $id => $item)
                    <div class="bg-white rounded-2xl p-4 md:p-6 shadow-sm border border-gray-100 flex flex-col md:flex-row items-center gap-6 group">
                        <!-- Image -->
                        <div class="w-24 h-24 md:w-32 md:h-32 rounded-xl overflow-hidden flex-shrink-0 bg-gray-50 border border-gray-100">
                            <img src="{{ $item['image'] ? asset('storage/' . $item['image']) : 'https://placehold.co/200x200/FDFCFB/C02424?text=' . urlencode($item['name']) }}" 
                                 alt="{{ $item['name'] }}" 
                        ...
                        <div class="flex-grow text-center md:text-left">
                            <h3 class="font-bold text-lg mb-1 group-hover:text-brand-red transition-colors">
                                <a href="/products/{{ $item['slug'] }}">{{ $item['name'] }}</a>
                            </h3>
                            <p class="text-brand-red font-bold">Rp {{ number_format($item['price'], 0, ',', '.') }}</p>
                        </div>

                        <!-- Quantity -->
                        <div class="flex items-center border border-gray-200 rounded-lg overflow-hidden bg-gray-50">
                            <button type="button" onclick="updateQty({{ $id }}, 'down')" class="px-3 py-1.5 hover:bg-gray-200 transition-colors">-</button>
                            <input type="number" value="{{ $item['quantity'] }}" min="1" class="w-10 text-center border-none bg-transparent focus:ring-0 font-bold pointer-events-none" id="qty-{{ $id }}">
                            <button type="button" onclick="updateQty({{ $id }}, 'up')" class="px-3 py-1.5 hover:bg-gray-200 transition-colors">+</button>
                        </div>
                        ...
                        <div class="text-right flex flex-col items-center md:items-end gap-2">
                            ...
                            <form action="{{ route('cart.remove', $id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-gray-400 hover:text-brand-red text-xs uppercase tracking-widest font-bold transition-colors">Hapus</button>
                            </form>
                        </div>
                    </div>
                @endforeach
            ...
            <div class="mt-8 bg-white rounded-3xl p-8 shadow-md border border-brand-gold/10">
                <div class="flex justify-between items-center mb-6">
                    <span class="text-gray-500 font-medium tracking-wide uppercase text-sm">Estimasi Total</span>
                    <span class="text-3xl font-serif font-bold text-brand-red">Rp {{ number_format($total, 0, ',', '.') }}</span>
                </div>

                @php
                    $waMsg = "Halo Batik Jambi Berkah, saya ingin memesan:\n\n";
        ...
        @else
            <div class="bg-white rounded-3xl p-16 text-center shadow-sm border border-gray-100">
                ...
                <h2 class="text-2xl font-serif mb-2">Keranjang Anda Kosong</h2>
                <p class="text-gray-400 mb-8">Wah, sepertinya Anda belum memilih batik favorit Anda.</p>
                <a href="/products" class="btn-primary inline-flex">Kembali Berbelanja</a>
            </div>
        @endif
    </div>
</section>
